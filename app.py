...
.trilha-mascote {
    ...
    animation: trilha-bounce 2.2s ease-in-out infinite;
}
@keyframes trilha-bounce {
...
# =========================================================
# TURSO VIA HTTP API (zero dependências extras)
# =========================================================
# Implementa o protocolo Hrana-HTTP do Turso usando só urllib (stdlib).
# Compatível com a interface sqlite3 — pode ser usado como drop-in replacement.

class TursoCursor:
    """Cursor que imita sqlite3.Cursor. Armazena resultado da última execução."""
    ...
    @staticmethod
    def _unpack_value(typed_value):
        """Converte um valor com tipo do Turso pro valor Python correspondente."""
        if typed_value is None:
            return None
        t = typed_value.get("type")
        v = typed_value.get("value")
        if t == "null":
            return None
        elif t == "integer":
            ...
            import base64
            return base64.b64decode(typed_value.get("base64", ""))
        else:  # text
            return v

    def execute(self, sql, params=()):
        """Executa SQL via HTTP. Retorna self pra permitir encadeamento (.fetchall())."""
        result = self._con._fazer_requisicao(sql, params)
        self._result = result
        if result:
            ...
            if lir is not None:
                ...
                except (TypeError, ValueError): self.lastrowid = None
            cols = result.get("cols", [])
            self.description = [(c.get("name"), None, None, None, None, None, None) for c in cols]
        return self

    def executemany(self, sql, params_list):
        """Executa SQL múltiplas vezes (uma vez por tupla de params)."""
    ...
    def fetchall(self):
        ...
        return [tuple(self._unpack_value(v) for v in row) for row in rows]
...
class TursoConnection:
    """Conexão HTTP com Turso que imita sqlite3.Connection."""

    def __init__(self, url, token):
        # Normaliza URL: libsql://X -> https://X
        u = url.strip()
        ...
        self.endpoint = self.url + "/v2/pipeline"

    def _converter_params(self, params):
        """Converte tupla de params pra formato Hrana ({type, value/base64})."""
        args = []
        for p in (params or ()):
            if p is None:
    ...
    def _fazer_requisicao(self, sql, params=()):
        """Faz POST /v2/pipeline com a query. Retorna o dict 'result' do response."""
        import urllib.request, urllib.error
        body = {
            "requests": [
                {"type": "execute", "stmt": {"sql": sql, "args": self._converter_params(params)}},
                {"type": "close"}
            ]
        }
        ...
        try:
            ...
            raise RuntimeError(f"Turso HTTP {e.code}: {corpo}")
        except urllib.error.URLError as e:
            raise RuntimeError(f"Turso conexão falhou: {e}")
    ...
    def execute(self, sql, params=()):
        """Imita Connection.execute(): cria cursor, executa, retorna cursor."""
    ...
    def commit(self):
        pass  # cada request é uma transação por padrão em Turso HTTP
...
TRILHA = [
    ...
    # ========================================================================
    # ===== EXPANSÃO: NÍVEL BÁSICO (módulos 39-48) =====
    # ========================================================================
    ("Módulo 39: Apresentação Pessoal", 1, [
        ("Fase 1", "Meu nome é João", "My name is John", "I name is John", "Me name is John", "My name am John"),
        ("Fase 2", "Eu sou do Brasil", "I am from Brazil", "I am of Brazil", "I am in Brazil", "I from Brazil"),
        ("Fase 3", "Eu tenho 25 anos", "I am 25 years old", "I have 25 years", "I have 25 years old", "I am 25 years"),
        ("Fase 4", "Prazer em conhecer você", "Nice to meet you", "Nice to know you", "Pleasure meet you", "Glad meeting you"),
        ("Fase 5", "Onde você mora?", "Where do you live?", "Where you live?", "Where lives you?", "Where are you live?"),
        ("Fase 6", "Eu moro em Marília", "I live in Marília", "I lives in Marília", "I am living Marília", "I live at Marília"),
        ("Fase 7", "Qual é a sua profissão?", "What's your job?", "What you job?", "What is you job?", "Which job you?"),
        ("Fase 8", "Eu sou estudante", "I am a student", "I am student", "I'm the student", "I am studant"),
    ]),
    ("Módulo 40: No Restaurante", 1, [
        ("Fase 1", "Posso ver o cardápio?", "Can I see the menu?", "Can I view the menu?", "Could I see menu?", "May see menu?"),
        ("Fase 2", "Eu gostaria de pedir", "I would like to order", "I want order", "I like to order", "I'd liking order"),
        ("Fase 3", "A conta, por favor", "The bill, please", "The check, please now", "Account, please", "Pay, please"),
        ("Fase 4", "Está delicioso!", "It's delicious!", "It is delicius!", "It's deliciously!", "Delicious it!"),
        ("Fase 5", "Eu sou vegetariano", "I am vegetarian", "I am a vegetarianer", "I'm vegetarianism", "I vegetarian"),
        ("Fase 6", "Eu quero um suco de laranja", "I want an orange juice", "I want a orange juice", "I want orange juice an", "I wanted orange juice"),
        ("Fase 7", "Sem açúcar, por favor", "No sugar, please", "Without sugar please", "Not sugar please", "Don't sugar please"),
        ("Fase 8", "Você tem opções sem glúten?", "Do you have gluten-free options?", "Have you gluten free?", "You have without gluten?", "Got you gluten free?"),
    ]),
    ("Módulo 41: Compras e Dinheiro", 1, [
        ("Fase 1", "Quanto custa?", "How much is it?", "How much costs?", "How is it much?", "How many is it?"),
        ("Fase 2", "É muito caro", "It's too expensive", "It's very expensive too", "It is expensive too much", "Too expensive it"),
        ("Fase 3", "Você aceita cartão?", "Do you accept cards?", "Accept you cards?", "You accept card?", "Have card you?"),
        ("Fase 4", "Estou só olhando", "I'm just looking", "I just looking", "I am only seeing", "I'm just seeing"),
        ("Fase 5", "Tem desconto?", "Is there a discount?", "Have discount?", "There is discount?", "Got a discount?"),
        ("Fase 6", "Eu vou levar", "I'll take it", "I will taking it", "I take it", "I'm take it"),
        ("Fase 7", "Tem em outro tamanho?", "Do you have another size?", "Have other size?", "Got another size you?", "You have other size?"),
        ("Fase 8", "Onde fica o caixa?", "Where is the cashier?", "Where the cashier?", "Where pay I?", "Where cashier is?"),
    ]),
    ("Módulo 42: Direções e Localização", 1, [
        ("Fase 1", "Vire à direita", "Turn right", "Go to right", "Turn the right", "Right turn now"),
        ("Fase 2", "Vire à esquerda", "Turn left", "Go to left", "Turn at left", "Left turn"),
        ("Fase 3", "Siga em frente", "Go straight", "Go ahead straight", "Walk forward", "Continue front"),
        ("Fase 4", "É perto daqui?", "Is it near here?", "It's close here?", "Near is it?", "Is near from here?"),
        ("Fase 5", "É longe", "It's far", "It's so far away too", "Far it is", "It is too far away"),
        ("Fase 6", "Ao lado do banco", "Next to the bank", "Beside the bank from", "On side of bank", "Near to bank"),
        ("Fase 7", "Atrás da escola", "Behind the school", "Back of school", "In back school", "Behind from school"),
        ("Fase 8", "Em frente ao mercado", "In front of the market", "Front the market", "Front of market", "In front market"),
    ]),
    ("Módulo 43: Hobbies e Tempo Livre", 1, [
        ("Fase 1", "Eu gosto de ler", "I like to read", "I like read", "I likes reading", "I am like read"),
        ("Fase 2", "Eu jogo futebol", "I play soccer", "I plays soccer", "I am playing soccer always", "I play to soccer"),
        ("Fase 3", "Eu assisto filmes", "I watch movies", "I see movies", "I watching movies", "I look movies"),
        ("Fase 4", "Eu toco violão", "I play guitar", "I touch guitar", "I plays the guitar", "I am play guitar"),
        ("Fase 5", "Eu adoro viajar", "I love traveling", "I love to travel always", "I love travelling so", "I am love travel"),
        ("Fase 6", "Eu desenho", "I draw", "I drawn", "I am drawing always", "I am drawn"),
        ("Fase 7", "Eu jogo videogame", "I play video games", "I plays games of video", "I play to videogame", "I'm play video game"),
        ("Fase 8", "Eu danço", "I dance", "I am dancer", "I'm dancing always", "I am to dance"),
    ]),
    ("Módulo 44: Esportes", 1, [
        ("Fase 1", "Futebol", "Soccer", "Football", "Foot ball", "Soccr"),
        ("Fase 2", "Basquete", "Basketball", "Basket ball", "Basketboll", "Basquet"),
        ("Fase 3", "Tênis", "Tennis", "Tenis", "Tennys", "Tenniss"),
        ("Fase 4", "Vôlei", "Volleyball", "Volley ball", "Voley", "Volyball"),
        ("Fase 5", "Natação", "Swimming", "Natation", "Swiming", "Swim"),
        ("Fase 6", "Corrida", "Running", "Run", "Runing", "Race"),
        ("Fase 7", "Ciclismo", "Cycling", "Bike riding", "Bicycling", "Bicycle"),
        ("Fase 8", "Boxe", "Boxing", "Box", "Boxe", "Boxer"),
    ]),
    ("Módulo 45: Frutas e Vegetais", 1, [
        ("Fase 1", "Maçã", "Apple", "Aple", "Appel", "Apel"),
        ("Fase 2", "Banana", "Banana", "Bananna", "Bannana", "Bananas"),
        ("Fase 3", "Laranja", "Orange", "Oranje", "Orangy", "Oring"),
        ("Fase 4", "Morango", "Strawberry", "Strawbery", "Straberry", "Strowberry"),
        ("Fase 5", "Uva", "Grape", "Grapes", "Graip", "Grap"),
        ("Fase 6", "Tomate", "Tomato", "Tomate", "Tomatoe", "Tomatto"),
        ("Fase 7", "Cenoura", "Carrot", "Carot", "Carrott", "Carrote"),
        ("Fase 8", "Alface", "Lettuce", "Letuce", "Letus", "Lattice"),
    ]),
    ("Módulo 46: Meios de Transporte", 1, [
        ("Fase 1", "Carro", "Car", "Cart", "Carr", "Karr"),
        ("Fase 2", "Ônibus", "Bus", "Buss", "Bues", "Boss"),
        ("Fase 3", "Avião", "Plane", "Airplain", "Plain", "Aeroplane"),
        ("Fase 4", "Trem", "Train", "Trein", "Trane", "Train"),
        ("Fase 5", "Bicicleta", "Bicycle", "Bycicle", "Bicicle", "Bisycle"),
        ("Fase 6", "Metrô", "Subway", "Sub way", "Subwey", "Underground way"),
        ("Fase 7", "Táxi", "Taxi", "Texi", "Taksi", "Taksy"),
        ("Fase 8", "Barco", "Boat", "Bote", "Boath", "Boot"),
    ]),
    ("Módulo 47: Móveis e Objetos da Casa", 1, [
        ("Fase 1", "Sofá", "Couch", "Sofa", "Sofah", "Sofs"),
        ("Fase 2", "Geladeira", "Fridge", "Frizer", "Frige", "Refridge"),
        ("Fase 3", "Fogão", "Stove", "Stov", "Cooker stove", "Estov"),
        ("Fase 4", "Televisão", "Television", "Telivision", "Televisao", "Televison"),
        ("Fase 5", "Chuveiro", "Shower", "Shawer", "Showr", "Showir"),
        ("Fase 6", "Janela", "Window", "Windou", "Window", "Wndow"),
        ("Fase 7", "Porta", "Door", "Dor", "Doore", "Doore"),
        ("Fase 8", "Espelho", "Mirror", "Miror", "Mirrour", "Mirroir"),
    ]),
    ("Módulo 48: Números 21-100", 1, [
        ("Fase 1", "Vinte e um", "Twenty-one", "Twentyone", "Twenty one", "Twentyone-"),
        ("Fase 2", "Trinta", "Thirty", "Thirteen", "Thurty", "Therty"),
        ("Fase 3", "Quarenta e cinco", "Forty-five", "Fourty five", "Fifty four", "Forty cinco"),
        ("Fase 4", "Cinquenta", "Fifty", "Fivty", "Fiveteen", "Fifteenth"),
        ("Fase 5", "Sessenta e três", "Sixty-three", "Sixty thirty", "Sixteen three", "Sixthy three"),
        ("Fase 6", "Setenta", "Seventy", "Seventeen", "Sevty", "Sevent"),
        ("Fase 7", "Oitenta e oito", "Eighty-eight", "Eighteen eight", "Eight eight", "Eithy eight"),
        ("Fase 8", "Cem", "One hundred", "A hundreds", "One hundreds", "Hundred one"),
    ]),
    # ========================================================================
    # ===== EXPANSÃO: NÍVEL INTERMEDIÁRIO (módulos 49-70) =====
    # ========================================================================
    ("Módulo 49: Present Continuous", 2, [
        ("Fase 1", "Eu estou estudando", "I am studying", "I studying", "I am study", "I am studied"),
        ("Fase 2", "Você está trabalhando?", "Are you working?", "You are working?", "Are you work?", "You working?"),
        ("Fase 3", "Ela está dormindo", "She is sleeping", "She sleep", "She sleeping", "She is sleep"),
        ("Fase 4", "Nós estamos comendo", "We are eating", "We eating", "We're eat", "We are eat"),
        ("Fase 5", "Eles estão correndo", "They are running", "They runing", "They are runned", "They running"),
        ("Fase 6", "Está chovendo agora", "It is raining now", "It rains now", "It's rain now", "It is rain now"),
        ("Fase 7", "Eu não estou assistindo", "I am not watching", "I no watching", "I don't watching", "I'm not watch"),
        ("Fase 8", "O que você está fazendo?", "What are you doing?", "What you doing?", "What do you doing?", "What is you doing?"),
    ]),
    ("Módulo 50: Past Continuous", 2, [
        ("Fase 1", "Eu estava dormindo", "I was sleeping", "I were sleeping", "I sleeping", "I was sleep"),
        ("Fase 2", "Eles estavam jogando", "They were playing", "They was playing", "They play", "They were play"),
        ("Fase 3", "Você estava estudando?", "Were you studying?", "Was you studying?", "You were study?", "Did you studying?"),
        ("Fase 4", "Ela estava cantando", "She was singing", "She were singing", "She singing", "She was sing"),
        ("Fase 5", "Estávamos esperando", "We were waiting", "We was waiting", "We waiting", "We were wait"),
        ("Fase 6", "Ele estava trabalhando", "He was working", "He were working", "He working", "He was work"),
        ("Fase 7", "Eu não estava ouvindo", "I wasn't listening", "I weren't listening", "I no listening", "I didn't listening"),
        ("Fase 8", "Estava chovendo ontem", "It was raining yesterday", "It rain yesterday", "It were raining yesterday", "It is raining yesterday"),
    ]),
    ("Módulo 51: Present Perfect", 2, [
        ("Fase 1", "Eu já comi", "I have eaten", "I already ate already", "I have ate", "I am eaten"),
        ("Fase 2", "Você já viu isso?", "Have you seen this?", "Did you saw this?", "You have see this?", "Have you saw this?"),
        ("Fase 3", "Ela nunca esteve aqui", "She has never been here", "She never was here", "She has never being here", "She doesn't been here"),
        ("Fase 4", "Eu acabei de chegar", "I have just arrived", "I just arrive", "I have just arriving", "I just arrived now"),
        ("Fase 5", "Nós estudamos por 5 anos", "We have studied for 5 years", "We studied for 5 years long", "We are studied 5 years", "We have study 5 years"),
        ("Fase 6", "Eles já foram pra Igreja?", "Have they been to church?", "Did they been church?", "They have went church?", "Were they to church?"),
        ("Fase 7", "Eu li este livro", "I have read this book", "I am read this book before", "I have readed this book", "I am readed this book"),
        ("Fase 8", "Você já viajou pra fora?", "Have you ever traveled abroad?", "Did you ever travel abroad?", "You have ever travel abroad?", "Did you traveled ever?"),
    ]),
    ("Módulo 52: Comparativos", 2, [
        ("Fase 1", "Maior que", "Bigger than", "More big than", "More bigger than", "Bigger that"),
        ("Fase 2", "Mais alto que", "Taller than", "More tall than", "More taller than", "Tallest than"),
        ("Fase 3", "Mais interessante", "More interesting", "Interestinger", "More interestinger", "Most interesting that"),
        ("Fase 4", "Melhor que", "Better than", "Gooder than", "More good than", "Best than"),
        ("Fase 5", "Pior que", "Worse than", "Badder than", "More bad than", "Worst than"),
        ("Fase 6", "Mais rápido", "Faster", "More fast", "Fasterer", "Most fast"),
        ("Fase 7", "Tão alto quanto", "As tall as", "So tall as", "Tall as", "More tall as"),
        ("Fase 8", "Menos caro que", "Less expensive than", "More less expensive than", "Less expensiver", "Cheaplier than"),
    ]),
    ("Módulo 53: Superlativos", 2, [
        ("Fase 1", "O maior", "The biggest", "The most big", "The bigger", "Biggest one"),
        ("Fase 2", "O mais inteligente", "The smartest", "The most smart", "Most smartest", "The smart one"),
        ("Fase 3", "O melhor", "The best", "The most good", "The gooder", "The bester"),
        ("Fase 4", "O pior", "The worst", "The most bad", "The badder", "The worster"),
        ("Fase 5", "O mais interessante", "The most interesting", "The interestingest", "The more interesting", "Interesting most"),
        ("Fase 6", "O mais rápido", "The fastest", "The most fast", "The faster", "Most fastest"),
        ("Fase 7", "O mais caro", "The most expensive", "The expensivest", "More expensive most", "Expensive most"),
        ("Fase 8", "O mais feliz", "The happiest", "The most happy", "The happyest", "Happiest most"),
    ]),
    ("Módulo 54: Artigos (a/an/the)", 2, [
        ("Fase 1", "Uma maçã", "An apple", "A apple", "The apple", "One apple"),
        ("Fase 2", "Um carro", "A car", "An car", "The a car", "One the car"),
        ("Fase 3", "Uma hora", "An hour", "A hour", "The hour one", "An hours"),
        ("Fase 4", "Uma universidade", "A university", "An university", "The university one", "An universite"),
        ("Fase 5", "O sol", "The sun", "A sun", "An sun", "Sun the"),
        ("Fase 6", "Um homem honesto", "An honest man", "A honest man", "The honest man one", "Honest a man"),
        ("Fase 7", "A internet", "The internet", "An internet", "A internet", "Internet the"),
        ("Fase 8", "Eu sou um professor", "I am a teacher", "I am the teacher of", "I am an teacher", "I am teacher"),
    ]),
    ("Módulo 55: Phrasal Verbs Básicos", 2, [
        ("Fase 1", "Acordar", "Wake up", "Wake on", "Get up wake", "Wake on up"),
        ("Fase 2", "Levantar (da cama)", "Get up", "Get on", "Stand up out", "Get out up"),
        ("Fase 3", "Vestir-se", "Get dressed", "Put dress", "Wear up", "Put on dress"),
        ("Fase 4", "Sentar-se", "Sit down", "Sit on", "Down sit", "Sit at"),
        ("Fase 5", "Desligar (luz)", "Turn off", "Turn out", "Stop off", "Off turn"),
        ("Fase 6", "Ligar (luz)", "Turn on", "Turn in", "On turn", "Start on"),
        ("Fase 7", "Vestir (roupa)", "Put on", "Put up", "Put in clothes", "Wear up"),
        ("Fase 8", "Tirar (roupa)", "Take off", "Take out", "Off take", "Remove out"),
    ]),
    ("Módulo 56: There is / There are", 2, [
        ("Fase 1", "Há um livro na mesa", "There is a book on the table", "It has a book on table", "There has a book table", "Have a book on table"),
        ("Fase 2", "Há muitas pessoas", "There are many people", "It has many people", "There has many peoples", "Have many people"),
        ("Fase 3", "Há um problema", "There is a problem", "It has problem", "There has problem", "Have problem"),
        ("Fase 4", "Não há comida", "There is no food", "There isn't food no", "Hasn't food", "Not have food"),
        ("Fase 5", "Há alguém aqui?", "Is there anyone here?", "Has anyone here?", "There is anyone?", "Have anyone here?"),
        ("Fase 6", "Havia uma festa ontem", "There was a party yesterday", "It had a party yesterday", "There had a party", "Had a party yesterday"),
        ("Fase 7", "Haverá uma reunião", "There will be a meeting", "It will have meeting", "Will have a meeting there", "There will has meeting"),
        ("Fase 8", "Há quanto tempo?", "How long has it been?", "How much time has?", "How long it has?", "How long is?"),
    ]),
    ("Módulo 57: Some / Any / No", 2, [
        ("Fase 1", "Eu tenho algum dinheiro", "I have some money", "I have any money", "I have no money some", "I have a money"),
        ("Fase 2", "Você tem algum amigo?", "Do you have any friends?", "Do you have some friends?", "You have any friend?", "Have you no friends?"),
        ("Fase 3", "Não tenho problema nenhum", "I have no problem", "I have any problem", "I not have problem none", "I have some problem"),
        ("Fase 4", "Há alguma novidade?", "Is there any news?", "Is there some news?", "There is no news?", "Have any news?"),
        ("Fase 5", "Quero um pouco de água", "I want some water", "I want any water", "I want water some", "I want a some water"),
        ("Fase 6", "Não há ninguém aqui", "There is no one here", "There is anyone here", "Have nobody here", "It has no one"),
        ("Fase 7", "Você tem algum tempo?", "Do you have any time?", "You have some time?", "Do you have no time?", "Have any time you?"),
        ("Fase 8", "Tem alguém na porta", "Someone is at the door", "Anyone is at door", "No one is at door", "Someone are at door"),
    ]),
    ("Módulo 58: Much / Many / A lot of", 2, [
        ("Fase 1", "Muita água", "Much water", "Many water", "Much waters", "A many water"),
        ("Fase 2", "Muitos amigos", "Many friends", "Much friends", "Manys friend", "A much friends"),
        ("Fase 3", "Muito dinheiro", "A lot of money", "A many money", "A lot money", "Lots money"),
        ("Fase 4", "Quanto dinheiro?", "How much money?", "How many money?", "How money much?", "How is money?"),
        ("Fase 5", "Quantos livros?", "How many books?", "How much books?", "How is books?", "How many book?"),
        ("Fase 6", "Não muitos", "Not many", "Not much", "No many", "Not too many no"),
        ("Fase 7", "Tempo demais", "Too much time", "Too many time", "Too time", "Many too time"),
        ("Fase 8", "Muitos problemas", "Many problems", "Much problems", "Manys problem", "A much problems"),
    ]),
    ("Módulo 59: Pronomes Possessivos", 2, [
        ("Fase 1", "Meu/Minha (com substantivo)", "My", "Mine", "Me", "I"),
        ("Fase 2", "É meu", "It's mine", "It's my", "It's me", "Mine it"),
        ("Fase 3", "Seu/Sua", "Your", "Yours", "You", "Yo"),
        ("Fase 4", "É seu", "It's yours", "It's your", "It's you", "Yours it"),
        ("Fase 5", "Dela", "Her / Hers", "She's", "Hers her", "Her of"),
        ("Fase 6", "Esse livro é dela", "This book is hers", "This book is her", "This is her book hers", "Hers book this"),
        ("Fase 7", "Nosso", "Our / Ours", "Us", "Wes", "Our of"),
        ("Fase 8", "A casa é nossa", "The house is ours", "The house is our", "The is our house", "Ours the house"),
    ]),
    ("Módulo 60: Demonstrativos (this/that/these/those)", 2, [
        ("Fase 1", "Este livro", "This book", "That book here", "These book", "This books"),
        ("Fase 2", "Aquele carro", "That car", "This car there", "Those car", "That cars"),
        ("Fase 3", "Estes meninos", "These boys", "This boys", "Thise boys", "Those boy these"),
        ("Fase 4", "Aquelas casas", "Those houses", "That houses", "Thoses houses", "These houses there"),
        ("Fase 5", "O que é isto?", "What is this?", "What is these?", "What this is?", "What's that this?"),
        ("Fase 6", "Estes são meus", "These are mine", "This are mine", "These is mine", "Those are my"),
        ("Fase 7", "Aquilo é interessante", "That is interesting", "Those is interesting", "This interesting", "It interesting that"),
        ("Fase 8", "Isso é incrível", "This is amazing", "These is amazing", "This amazing", "That are amazing"),
    ]),
    ("Módulo 61: Advérbios de Frequência", 2, [
        ("Fase 1", "Sempre", "Always", "All time", "Allways", "Ever"),
        ("Fase 2", "Eu sempre oro", "I always pray", "I pray always", "I pray every", "Always I pray"),
        ("Fase 3", "Geralmente", "Usually", "Usuelly", "Usual", "Often always"),
        ("Fase 4", "Frequentemente", "Often", "Ofthen", "Oft", "Frequenty"),
        ("Fase 5", "Às vezes", "Sometimes", "Sometime", "Some time", "Times some"),
        ("Fase 6", "Raramente", "Rarely", "Rare", "Rarly", "Hardly never"),
        ("Fase 7", "Nunca", "Never", "Nover", "Not ever never", "Ever no"),
        ("Fase 8", "Quase nunca", "Hardly ever", "Hard ever", "Almost never ever", "Hardly never"),
    ]),
    ("Módulo 62: Preposições de Tempo (in/on/at)", 2, [
        ("Fase 1", "Em julho", "In July", "On July", "At July", "By July"),
        ("Fase 2", "Em 2024", "In 2024", "On 2024", "At 2024", "By 2024"),
        ("Fase 3", "Na segunda-feira", "On Monday", "In Monday", "At Monday", "By Monday"),
        ("Fase 4", "No Natal", "At Christmas", "On Christmas", "In Christmas", "By Christmas"),
        ("Fase 5", "Às 7 horas", "At 7 o'clock", "On 7 o'clock", "In 7 o'clock", "By 7 o'clock"),
        ("Fase 6", "De manhã", "In the morning", "On the morning", "At the morning", "By morning"),
        ("Fase 7", "À noite", "At night", "On night", "In night", "By night"),
        ("Fase 8", "Em 5 minutos", "In 5 minutes", "On 5 minutes", "At 5 minutes", "After 5 minutes only"),
    ]),
    ("Módulo 63: Question Tags", 2, [
        ("Fase 1", "Você é estudante, não é?", "You are a student, aren't you?", "You are student, no?", "You student, isn't?", "Are you student, are you?"),
        ("Fase 2", "Ele não veio, veio?", "He didn't come, did he?", "He no come, came?", "Didn't he come, did?", "He didn't come, isn't he?"),
        ("Fase 3", "Vamos pra igreja, vamos?", "Let's go to church, shall we?", "Let's go to church, will we?", "Go we to church, no?", "Let's church, do we?"),
        ("Fase 4", "Você gosta de café, não gosta?", "You like coffee, don't you?", "You like coffee, isn't?", "You likes coffee, no?", "Do you like coffee, you?"),
        ("Fase 5", "Ela é bonita, não é?", "She is pretty, isn't she?", "She is pretty, no?", "Pretty she, isn't?", "She pretty, is she?"),
        ("Fase 6", "Eles podem vir, não podem?", "They can come, can't they?", "They can come, don't they?", "Can they come, can?", "They come, can they?"),
        ("Fase 7", "Você tem 20 anos, não tem?", "You're 20, aren't you?", "You have 20, no?", "You 20 years, isn't?", "Are you 20, do you?"),
        ("Fase 8", "Não vai chover, vai?", "It won't rain, will it?", "It no rain, rains?", "Won't it rain, won't?", "It will not rain, won't?"),
    ]),
    ("Módulo 64: Used to", 2, [
        ("Fase 1", "Eu costumava jogar", "I used to play", "I use to play", "I am used to play", "I used playing"),
        ("Fase 2", "Ela costumava morar aqui", "She used to live here", "She use to live here", "She used living here", "She is used to live here"),
        ("Fase 3", "Você costumava fumar?", "Did you use to smoke?", "Did you used to smoke?", "Used you to smoke?", "Were you use to smoke?"),
        ("Fase 4", "Eu não costumava gostar", "I didn't use to like it", "I didn't used to like", "I don't use to like", "I didn't liked use to"),
        ("Fase 5", "Eles costumavam ir à igreja", "They used to go to church", "They use go to church", "They were used to go church", "They used going to church"),
        ("Fase 6", "Eu costumava ser tímido", "I used to be shy", "I use to be shy", "I was used to shy", "I used to being shy"),
        ("Fase 7", "Estou acostumado a isso", "I'm used to it", "I used to it", "I am use to it", "I'm used at it"),
        ("Fase 8", "Não estou acostumada", "I'm not used to it", "I not used to it", "I'm not use to it", "I don't used to it"),
    ]),
    ("Módulo 65: First Conditional", 2, [
        ("Fase 1", "Se chover, ficarei em casa", "If it rains, I will stay home", "If it will rain, I stay home", "If rains, I'll stay home", "If it rain, I'll stay"),
        ("Fase 2", "Se eu tiver tempo, ajudo", "If I have time, I'll help", "If I will have time, I help", "If I had time, I'll help", "If I have time, I helped"),
        ("Fase 3", "Se você estudar, passará", "If you study, you'll pass", "If you'll study, you pass", "If you studying, you'll pass", "If you studied, you'll pass"),
        ("Fase 4", "Se ele vier, falo com ele", "If he comes, I'll talk to him", "If he will come, I talk to him", "If he came, I talk to him", "If he come, I'll talk"),
        ("Fase 5", "Se Deus quiser, irei", "If God wills, I'll go", "If God will want, I'll go", "If God wanted, I'll go", "If God wishing, I'll go"),
        ("Fase 6", "Se ela me ligar, atendo", "If she calls me, I'll answer", "If she will call, I answer", "If she calling me, I'll answer", "If she called, I'll answer"),
        ("Fase 7", "Se você precisar, me chame", "If you need, call me", "If you needed, call me", "If you'll need, call me", "If you needing, call me"),
        ("Fase 8", "Se eu puder, vou ajudar", "If I can, I'll help", "If I will can, I help", "If I could, I'll help", "If I'm able, I helping"),
    ]),
    ("Módulo 66: Vocabulário Educação", 2, [
        ("Fase 1", "Escola", "School", "Skool", "Schol", "Schoole"),
        ("Fase 2", "Universidade", "University", "Universit", "Universety", "Universidad"),
        ("Fase 3", "Sala de aula", "Classroom", "Class room", "Classrum", "Roomclass"),
        ("Fase 4", "Lição de casa", "Homework", "Home work", "Homewerk", "Workhome"),
        ("Fase 5", "Prova", "Exam", "Examen", "Exame", "Examp"),
        ("Fase 6", "Caderno", "Notebook", "Note book", "Notbook", "Notibook"),
        ("Fase 7", "Diretor", "Principal", "Director", "Principa", "Directorr"),
        ("Fase 8", "Bolsa de estudos", "Scholarship", "Scolarship", "Scholar ship", "Schoolarship"),
    ]),
    ("Módulo 67: Saúde e Médico", 2, [
        ("Fase 1", "Estou doente", "I'm sick", "I am sicked", "I have sick", "I sick"),
        ("Fase 2", "Estou com dor de cabeça", "I have a headache", "I have headache", "I am with headache", "I am head pain"),
        ("Fase 3", "Você precisa de remédio", "You need medicine", "You need medecine", "You needs medicine", "You medicine need"),
        ("Fase 4", "Estou com febre", "I have a fever", "I am fever", "I have feber", "I am with fever"),
        ("Fase 5", "Onde fica o hospital?", "Where is the hospital?", "Where the hospital?", "Where lives hospital?", "Where hospital is at?"),
        ("Fase 6", "Preciso ver um médico", "I need to see a doctor", "I need see doctor", "I needing see doctor", "I needed see doctor"),
        ("Fase 7", "Estou melhor", "I'm feeling better", "I feel more good", "I am betterer", "I'm feel best"),
        ("Fase 8", "Cuide-se!", "Take care!", "Take cares!", "Cares!", "Care you!"),
    ]),
    ("Módulo 68: Vocabulário de Viagem", 2, [
        ("Fase 1", "Passaporte", "Passport", "Pass port", "Passporte", "Pasport"),
        ("Fase 2", "Bagagem", "Luggage", "Lugage", "Luggages", "Bagage"),
        ("Fase 3", "Aeroporto", "Airport", "Air port", "Aeroport", "Airporth"),
        ("Fase 4", "Hotel", "Hotel", "Otel", "Hotell", "Hotels"),
        ("Fase 5", "Reserva", "Reservation", "Reservacion", "Reserve", "Reservetion"),
        ("Fase 6", "Embarque", "Boarding", "Embark", "Embarque", "Boardign"),
        ("Fase 7", "Voo", "Flight", "Flite", "Fly", "Fligth"),
        ("Fase 8", "Atraso", "Delay", "Late", "Delayed", "Delai"),
    ]),
    ("Módulo 69: Trabalho e Escritório", 2, [
        ("Fase 1", "Reunião", "Meeting", "Meetting", "Meet", "Meting"),
        ("Fase 2", "Chefe", "Boss", "Bos", "Bose", "Bouss"),
        ("Fase 3", "Funcionário", "Employee", "Employ", "Employer", "Emploiee"),
        ("Fase 4", "Salário", "Salary", "Salery", "Salay", "Sallary"),
        ("Fase 5", "Demitir", "Fire", "Demite", "Dismiss out", "Layoff"),
        ("Fase 6", "Contratar", "Hire", "Contract", "Higher", "Hiring"),
        ("Fase 7", "Currículo", "Resume", "Curriculum", "Resumee", "Curiculo"),
        ("Fase 8", "Entrevista", "Interview", "Inter view", "Intervue", "Intervieuw"),
    ]),
    ("Módulo 70: Tecnologia", 2, [
        ("Fase 1", "Computador", "Computer", "Computor", "Komputer", "Computere"),
        ("Fase 2", "Celular", "Cell phone", "Cellphone", "Cell fone", "Cellular phone"),
        ("Fase 3", "Site/Página", "Website", "Web site", "Websight", "Wevsite"),
        ("Fase 4", "Senha", "Password", "Pass word", "Pasword", "Passwerd"),
        ("Fase 5", "Baixar (download)", "Download", "Down load", "Donwload", "Downloaded"),
        ("Fase 6", "Enviar email", "Send an email", "Send email", "Send a email", "Sending email"),
        ("Fase 7", "Conectar-se", "Connect", "Conect", "Conection", "Connect to"),
        ("Fase 8", "Atualizar", "Update", "Up date", "Updat", "Updaite"),
    ]),
    # ========================================================================
    # ===== EXPANSÃO: NÍVEL AVANÇADO (módulos 71-87) =====
    # ========================================================================
    ("Módulo 71: Present Perfect Continuous", 3, [
        ("Fase 1", "Eu estou estudando há 2 horas", "I have been studying for 2 hours", "I am studying since 2 hours", "I have studied since 2 hours", "I'm studying for 2 hours"),
        ("Fase 2", "Está chovendo o dia todo", "It has been raining all day", "It is raining all day long", "It rains all day", "It has rain all day"),
        ("Fase 3", "Eles estão esperando há muito tempo", "They have been waiting for a long time", "They are waiting long time", "They wait long time", "They have been wait long"),
        ("Fase 4", "Há quanto tempo você mora aqui?", "How long have you been living here?", "How long do you live here?", "How long are you living here?", "How long lived you here?"),
        ("Fase 5", "Ela está trabalhando muito", "She has been working hard", "She is working hard since", "She has worked hard since now", "She worked hard so"),
        ("Fase 6", "Estamos orando por isso", "We have been praying for this", "We are praying since this", "We prayed for this since", "We been praying this"),
        ("Fase 7", "Você está dormindo bem?", "Have you been sleeping well?", "Are you sleeping well since?", "Do you have slept well?", "Were you sleeping well?"),
        ("Fase 8", "Eu não tenho dormido bem", "I haven't been sleeping well", "I am not sleeping well", "I don't sleep well since", "I haven't slept well since"),
    ]),
    ("Módulo 72: Past Perfect", 3, [
        ("Fase 1", "Eu já tinha comido", "I had already eaten", "I have already ate", "I had ate already", "I already ate had"),
        ("Fase 2", "Ela já tinha saído", "She had already left", "She have left already", "She had leaved already", "She already had leave"),
        ("Fase 3", "Quando cheguei, eles tinham ido", "When I arrived, they had gone", "When I arrived, they have gone", "When I arrive, they had go", "When I arrived, they were gone had"),
        ("Fase 4", "Eu nunca tinha visto isso", "I had never seen that", "I have never see that", "I never had seen that", "I had never saw that"),
        ("Fase 5", "Ele tinha estudado por anos", "He had studied for years", "He have studied for years", "He had study for years", "He has studied years"),
        ("Fase 6", "Nós tínhamos feito tudo", "We had done everything", "We have done all things", "We had do everything", "We did had everything"),
        ("Fase 7", "Ela já tinha lido o livro", "She had already read the book", "She has already read the book", "She had read book already", "She already had read book"),
        ("Fase 8", "Você já tinha estado lá antes?", "Had you been there before?", "Have you been there before?", "Did you been there before?", "Were you been there before?"),
    ]),
    ("Módulo 73: Future Perfect", 3, [
        ("Fase 1", "Eu terei terminado até sexta", "I will have finished by Friday", "I will finished by Friday", "I will have finish by Friday", "I would have finished Friday"),
        ("Fase 2", "Ela já terá saído", "She will have already left", "She will already leave", "She will have left already gone", "She would have already leave"),
        ("Fase 3", "Em 5 anos, terei me formado", "In 5 years, I will have graduated", "In 5 years, I will graduate", "In 5 years, I would graduate", "In 5 years, I have graduated"),
        ("Fase 4", "Teremos viajado muito", "We will have traveled a lot", "We will travel a lot", "We had traveled a lot", "We would have travel a lot"),
        ("Fase 5", "Ele já terá lido até amanhã", "He will have read it by tomorrow", "He will read it by tomorrow", "He would have read by tomorrow", "He has read it by tomorrow"),
        ("Fase 6", "Vamos ter chegado às 8h", "We will have arrived by 8", "We will arrive by 8 already", "We would arrive by 8", "We will be arrived by 8"),
        ("Fase 7", "Você terá completado tudo?", "Will you have completed everything?", "Will you complete all?", "Would you completed all?", "Have you complete all?"),
        ("Fase 8", "Eles terão construído a igreja", "They will have built the church", "They will build the church already", "They would have built church", "They will be built the church"),
    ]),
    ("Módulo 74: Second Conditional", 3, [
        ("Fase 1", "Se eu fosse rico, viajaria", "If I were rich, I would travel", "If I was rich, I will travel", "If I am rich, I would travel", "If I would be rich, I traveled"),
        ("Fase 2", "Se ela soubesse, ajudaria", "If she knew, she would help", "If she knows, she would help", "If she would know, she helped", "If she knew, she will help"),
        ("Fase 3", "Se eu pudesse, iria com você", "If I could, I would go with you", "If I can, I would go with you", "If I would can, I go with you", "If I could, I will go you"),
        ("Fase 4", "O que você faria?", "What would you do?", "What will you do?", "What do you would do?", "What would do you?"),
        ("Fase 5", "Eu compraria uma casa", "I would buy a house", "I will buy a house", "I would to buy a house", "I bought would a house"),
        ("Fase 6", "Se ele estivesse aqui...", "If he were here...", "If he was here", "If he would be here", "If he is here"),
        ("Fase 7", "Se eu fosse você, oraria", "If I were you, I would pray", "If I was you, I will pray", "If I am you, I pray", "If I would be you, I prayed"),
        ("Fase 8", "Onde você moraria?", "Where would you live?", "Where will you live?", "Where do you would live?", "Where would live you?"),
    ]),
    ("Módulo 75: Third Conditional", 3, [
        ("Fase 1", "Se eu tivesse estudado, teria passado", "If I had studied, I would have passed", "If I have studied, I would pass", "If I studied, I would have passed", "If I had study, I would have pass"),
        ("Fase 2", "Se ela tivesse vindo, teria visto", "If she had come, she would have seen", "If she came, she would have seen", "If she has come, she would seen", "If she had came, she would seen"),
        ("Fase 3", "Eu teria ido se tivesse tempo", "I would have gone if I had had time", "I would go if I had time", "I will have go if I had time", "I would have gone if I had time"),
        ("Fase 4", "Se você tivesse pedido, eu teria ajudado", "If you had asked, I would have helped", "If you asked, I would have helped", "If you have asked, I would help", "If you had asked, I would helped"),
        ("Fase 5", "Eles teriam ganhado", "They would have won", "They would win", "They had would won", "They would have win"),
        ("Fase 6", "Se não tivesse chovido...", "If it hadn't rained...", "If it didn't rain", "If it didn't rained", "If it doesn't have rained"),
        ("Fase 7", "O que teria acontecido?", "What would have happened?", "What would happen?", "What had happened would?", "What would have happen?"),
        ("Fase 8", "Eu teria orado por ele", "I would have prayed for him", "I would pray for him", "I had prayed for him", "I would have pray for him"),
    ]),
    ("Módulo 76: Mixed Conditionals", 3, [
        ("Fase 1", "Se eu tivesse estudado, saberia agora", "If I had studied, I would know now", "If I studied, I would know", "If I have studied, I will know now", "If I had study, I would knowing"),
        ("Fase 2", "Se eu fosse rico, teria comprado", "If I were rich, I would have bought it", "If I was rich, I would buy it", "If I would be rich, I bought it", "If I were rich, I would buy it have"),
        ("Fase 3", "Se ela morasse aqui, teria me visto", "If she lived here, she would have seen me", "If she lives here, she would have seen me", "If she lived here, she would see me", "If she would live, she seen me"),
        ("Fase 4", "Se eu não fosse cristão, não teria paz", "If I weren't a Christian, I wouldn't have peace", "If I am not Christian, I wouldn't peace", "If I wasn't Christian, I would have not peace", "If I were not Christian, I have no peace"),
        ("Fase 5", "Estaria feliz se tivesse passado", "I would be happy if I had passed", "I will be happy if I had passed", "I would been happy if passed", "I would be happy if I would have passed"),
        ("Fase 6", "Se ele tivesse trabalhado mais, seria rico", "If he had worked more, he would be rich", "If he worked more, he would been rich", "If he has worked more, he would be rich", "If he would have worked more, he is rich"),
        ("Fase 7", "Saberia inglês se tivesse estudado", "I would know English if I had studied", "I would know English if I studied", "I would knowing English if I studied", "I would have known English if I studied"),
        ("Fase 8", "Se fôssemos mais sábios, teríamos evitado", "If we were wiser, we would have avoided it", "If we are wise, we would avoided", "If we wiser, we would avoid it", "If we would been wiser, we avoided"),
    ]),
    ("Módulo 77: Voz Passiva - Presente", 3, [
        ("Fase 1", "O livro é lido por mim", "The book is read by me", "The book reads by me", "The book is reading by me", "The book read me"),
        ("Fase 2", "As janelas são limpas", "The windows are cleaned", "The windows clean", "The windows is cleaned", "The windows are clean by"),
        ("Fase 3", "A música é tocada", "The song is played", "The song plays", "The song is play", "The song is playing by"),
        ("Fase 4", "Inglês é falado aqui", "English is spoken here", "English speaks here", "English is speak here", "English is speaking here"),
        ("Fase 5", "Cartas são enviadas todos os dias", "Letters are sent every day", "Letters sent every day", "Letters are send every day", "Letters is sent every day"),
        ("Fase 6", "A igreja é construída", "The church is built", "The church builds", "The church is build", "The church is building by"),
        ("Fase 7", "Deus é amado", "God is loved", "God loves", "God is love", "God is loving by all"),
        ("Fase 8", "A reunião é cancelada", "The meeting is canceled", "The meeting cancels", "The meeting is cancel", "The meeting cancel is"),
    ]),
    ("Módulo 78: Voz Passiva - Passado", 3, [
        ("Fase 1", "O livro foi escrito", "The book was written", "The book wrote", "The book was wrote", "The book is written was"),
        ("Fase 2", "Os ladrões foram presos", "The thieves were arrested", "The thieves arrested", "The thieves was arrested", "The thieves were arrest"),
        ("Fase 3", "A torta foi feita por ela", "The pie was made by her", "The pie made by her", "The pie was making by her", "The pie was made for her"),
        ("Fase 4", "A casa foi vendida", "The house was sold", "The house sold", "The house was sell", "The house is sold was"),
        ("Fase 5", "Cartas foram entregues", "Letters were delivered", "Letters delivered", "Letters was delivered", "Letters were deliver"),
        ("Fase 6", "Jesus foi crucificado", "Jesus was crucified", "Jesus crucified", "Jesus was crucify", "Jesus was crucifying"),
        ("Fase 7", "O templo foi destruído", "The temple was destroyed", "The temple destroyed", "The temple was destroy", "The temple destroyed was"),
        ("Fase 8", "Eles foram convidados", "They were invited", "They invited", "They was invited", "They were invite"),
    ]),
    ("Módulo 79: Reported Speech", 3, [
        ("Fase 1", "Ele disse que estava cansado", "He said he was tired", "He said he is tired", "He said that he tired", "He told he was tired"),
        ("Fase 2", "Ela disse que viria", "She said she would come", "She said she will come", "She said that she come", "She tell she will come"),
        ("Fase 3", "Eles disseram que tinham ido", "They said they had gone", "They said they have gone", "They said they went", "They told they had gone"),
        ("Fase 4", "Ele perguntou onde eu morava", "He asked where I lived", "He asked where I live", "He asked where do I live", "He ask where I lived"),
        ("Fase 5", "Ela me disse para esperar", "She told me to wait", "She said me wait", "She told me wait", "She told to me wait"),
        ("Fase 6", "Ele me pediu pra ajudar", "He asked me to help", "He asked me help", "He asked to me help", "He ask me help"),
        ("Fase 7", "Eu disse que oraria", "I said I would pray", "I said I will pray", "I told I would pray", "I said that I pray"),
        ("Fase 8", "Ela perguntou se eu estava bem", "She asked if I was okay", "She asked if I am okay", "She asked am I okay", "She asks if I was okay"),
    ]),
    ("Módulo 80: Relative Clauses", 3, [
        ("Fase 1", "O homem que mora aqui", "The man who lives here", "The man which lives here", "The man what lives here", "The man whose lives here"),
        ("Fase 2", "O livro que eu li", "The book that I read", "The book who I read", "The book which read I", "The book what I read"),
        ("Fase 3", "A mulher cujo filho é médico", "The woman whose son is a doctor", "The woman that son is doctor", "The woman which son is doctor", "The woman who son is doctor"),
        ("Fase 4", "A casa onde nasci", "The house where I was born", "The house which I was born", "The house that I was born", "The house in where I born"),
        ("Fase 5", "O ano em que nos conhecemos", "The year when we met", "The year that we met", "The year where we met", "The year which we met"),
        ("Fase 6", "Pessoas que oram", "People who pray", "People which pray", "People what pray", "People whose pray"),
        ("Fase 7", "Tudo que ele disse", "Everything he said", "Everything what he said", "Everything who he said", "Everything whom he said"),
        ("Fase 8", "Esse é o motivo pelo qual fui", "That's the reason why I went", "That's the reason that I went", "That's the reason which I went", "That's the reason where I went"),
    ]),
    ("Módulo 81: Wish + Past", 3, [
        ("Fase 1", "Eu queria ser mais alto", "I wish I were taller", "I wish I am taller", "I wish I be taller", "I wish I am more tall"),
        ("Fase 2", "Queria que ela estivesse aqui", "I wish she were here", "I wish she is here", "I wish she be here", "I wish she has here"),
        ("Fase 3", "Eu queria saber inglês", "I wish I knew English", "I wish I know English", "I wish I will know English", "I wish to know English"),
        ("Fase 4", "Ele queria ter mais tempo", "He wishes he had more time", "He wishes he has more time", "He wishes he will have more time", "He wishes more time"),
        ("Fase 5", "Queria não ter feito isso", "I wish I hadn't done that", "I wish I didn't do that", "I wish I haven't done that", "I wish I don't do that"),
        ("Fase 6", "Queria que parasse de chover", "I wish it would stop raining", "I wish it stops raining", "I wish it stop raining", "I wish stopping rain"),
        ("Fase 7", "Eu queria poder voar", "I wish I could fly", "I wish I can fly", "I wish I will fly", "I wish to fly"),
        ("Fase 8", "Ela queria ter ido", "She wishes she had gone", "She wishes she went", "She wishes she goes", "She wishes she would go"),
    ]),
    ("Módulo 82: Causative (have/get done)", 3, [
        ("Fase 1", "Mandei cortar meu cabelo", "I had my hair cut", "I cut my hair", "I had cut my hair", "I have cut my hair"),
        ("Fase 2", "Vou mandar consertar o carro", "I will get my car repaired", "I will repair my car", "I will get my car repair", "I'll have car repaired"),
        ("Fase 3", "Ela mandou pintar a casa", "She had her house painted", "She painted the house", "She had paint her house", "She has paint her house"),
        ("Fase 4", "Eu mandei fazer um bolo", "I had a cake made", "I made a cake", "I had make a cake", "I had cake make"),
        ("Fase 5", "Você precisa cortar o cabelo", "You need to get your hair cut", "You need to cut hair", "You need cut your hair", "You need cutting hair"),
        ("Fase 6", "Mandei limpar o sofá", "I had the couch cleaned", "I cleaned the couch", "I had clean the couch", "I had couch clean"),
        ("Fase 7", "Vou mandar fazer um vestido", "I'm going to have a dress made", "I'm going to make a dress", "I'm going to have make dress", "I will have a dress make"),
        ("Fase 8", "Ele mandou consertar o relógio", "He had his watch fixed", "He fixed his watch", "He had fix watch", "He has fix his watch"),
    ]),
    ("Módulo 83: Phrasal Verbs Avançados 1", 3, [
        ("Fase 1", "Descobrir", "Find out", "Find in", "Discover out", "Find about"),
        ("Fase 2", "Desistir", "Give up", "Give out", "Quit up", "Give in up"),
        ("Fase 3", "Aparecer (de repente)", "Show up", "Show in", "Appear up", "Show on"),
        ("Fase 4", "Cancelar", "Call off", "Call out", "Cancel off", "Stop off"),
        ("Fase 5", "Cuidar de", "Look after", "Look at after", "Care of", "See after"),
        ("Fase 6", "Inventar (uma história)", "Make up", "Make in", "Invent up", "Create up"),
        ("Fase 7", "Recusar", "Turn down", "Turn off", "Refuse down", "Turn out"),
        ("Fase 8", "Ficar sem (algo)", "Run out of", "Run out", "Run from", "Run of"),
    ]),
    ("Módulo 84: Phrasal Verbs Avançados 2", 3, [
        ("Fase 1", "Entrar em (carro pequeno)", "Get in", "Get into into", "Enter in", "Get on"),
        ("Fase 2", "Entrar em (transporte público)", "Get on", "Get in", "Enter on", "Climb on"),
        ("Fase 3", "Continuar", "Carry on", "Carry up", "Continue on now", "Keep going on so"),
        ("Fase 4", "Sair com (alguém)", "Go out with", "Go with", "Out with go", "Go out"),
        ("Fase 5", "Voltar atrás (palavra)", "Go back on", "Go back", "Back on go", "Take back on"),
        ("Fase 6", "Encarar", "Face up to", "Face to", "Face up", "Face for"),
        ("Fase 7", "Bater em (carro)", "Run into", "Run in", "Hit into", "Run on"),
        ("Fase 8", "Tropeçar em", "Come across", "Come over", "Trip across", "Cross come"),
    ]),
    ("Módulo 85: Idioms Básicos", 3, [
        ("Fase 1", "Está chovendo muito", "It's raining cats and dogs", "It's raining cat dog", "It rains pets", "Cats dogs raining"),
        ("Fase 2", "Quebrar a perna! (Boa sorte)", "Break a leg!", "Lucky leg!", "Break leg good!", "Good leg luck!"),
        ("Fase 3", "Custar uma fortuna", "Cost an arm and a leg", "Cost arm leg", "Cost expensive much", "Cost like a fortune leg"),
        ("Fase 4", "De vez em quando", "Once in a while", "Once at a while", "One in while", "Once when while"),
        ("Fase 5", "Pôr a mão na massa", "Get your hands dirty", "Put hand mass", "Hands in mass", "Dirty work hand"),
        ("Fase 6", "Matar dois coelhos com uma cajadada", "Kill two birds with one stone", "Kill rabbits with stick", "Two birds one cane", "Hit two with one"),
        ("Fase 7", "Pedaço de bolo (fácil)", "A piece of cake", "Easy cake piece", "Cake piece is", "A cake easy slice"),
        ("Fase 8", "No fundo", "Deep down", "In deep", "Down deep up", "Deeply"),
    ]),
    ("Módulo 86: Business English", 3, [
        ("Fase 1", "Reunião de negócios", "Business meeting", "Business meet", "Bizness meeting", "Meeting business"),
        ("Fase 2", "Prazo final", "Deadline", "Dead line line", "Time end", "End time line"),
        ("Fase 3", "Orçamento", "Budget", "Buget", "Bujet", "Budgett"),
        ("Fase 4", "Lucro", "Profit", "Profite", "Profet", "Earning profit"),
        ("Fase 5", "Investimento", "Investment", "Invest", "Investiment", "Investmnt"),
        ("Fase 6", "Fechar negócio", "Close a deal", "Close deal", "Closed deal", "Make a deal close"),
        ("Fase 7", "Acionista", "Shareholder", "Sharer", "Stock holder share", "Action holder"),
        ("Fase 8", "Lançamento (produto)", "Launch", "Lanch", "Launched", "Launching"),
    ]),
    ("Módulo 87: Vocabulário Acadêmico", 3, [
        ("Fase 1", "Pesquisa", "Research", "Reasearch", "Research study now", "Reserch"),
        ("Fase 2", "Hipótese", "Hypothesis", "Hipothesis", "Hypothese", "Hipotesis"),
        ("Fase 3", "Conclusão", "Conclusion", "Conclution", "Concluson", "Conclusion of"),
        ("Fase 4", "Análise", "Analysis", "Analisis", "Analyses", "Analize"),
        ("Fase 5", "Tese", "Thesis", "Tesis", "Thesys", "Thesisses"),
        ("Fase 6", "Bibliografia", "Bibliography", "Biblography", "Bibliografy", "Bibliograhy"),
        ("Fase 7", "Citação", "Citation", "Citacion", "Citatation", "Quoting"),
        ("Fase 8", "Resumo", "Abstract", "Abstrat", "Resumee", "Summary abstract of"),
    ]),
    # ========================================================================
    # ===== EXPANSÃO: NÍVEL FLUENTE (módulos 88-94) - C2 =====
    # ========================================================================
    ("Módulo 88: Inversion (Inversão)", 3, [
        ("Fase 1", "Nunca tinha visto algo assim", "Never had I seen such a thing", "Never I had seen such thing", "I had never seen thing such", "Never have I seen such thing"),
        ("Fase 2", "Mal tinha chegado quando ela ligou", "Hardly had I arrived when she called", "Hardly I had arrived when she called", "I hardly had arrived when she call", "Hardly had I arrive when she called"),
        ("Fase 3", "Não só ela cantou, como dançou", "Not only did she sing, but she also danced", "Not only she sang, but she danced", "Not only she sing, but she danced", "Not only sang she, but she danced"),
        ("Fase 4", "Em nenhuma circunstância faríamos isso", "Under no circumstances would we do that", "Under no circumstances we would do that", "Under no circumstances we did that", "In no circumstances we will do that"),
        ("Fase 5", "Pouco sabia ele do que viria", "Little did he know what was coming", "Little he did know what coming", "Little he knew what was coming", "Little knew he what came"),
        ("Fase 6", "Tão alto ele falou que todos ouviram", "So loudly did he speak that everyone heard", "So loudly he spoke that everyone heard", "So loudly did he spoke that all heard", "So loud he speak that all heard"),
        ("Fase 7", "Apenas depois é que entendi", "Only later did I understand", "Only later I understood", "Only later I did understood", "Only later understood I"),
        ("Fase 8", "Raramente vejo tal beleza", "Rarely do I see such beauty", "Rarely I see such beauty", "Rarely I do see such beauty", "Rarely see I beauty such"),
    ]),
    ("Módulo 89: Modo Subjuntivo", 3, [
        ("Fase 1", "Eu sugiro que ele estude", "I suggest that he study", "I suggest that he studies", "I suggest he is studying", "I suggest him to study"),
        ("Fase 2", "É essencial que sejamos pontuais", "It's essential that we be punctual", "It's essential that we are punctual", "It's essential we to be punctual", "Essential we are punctual"),
        ("Fase 3", "Se eu fosse rei...", "If I were king...", "If I was king", "If I would be king", "If I being king"),
        ("Fase 4", "Recomendo que ele venha", "I recommend that he come", "I recommend that he comes", "I recommend him to coming", "I recommend he is coming"),
        ("Fase 5", "Deus queira que sim", "God willing", "God wanting", "If God wants", "God wishes"),
        ("Fase 6", "Que assim seja", "So be it", "So is it", "So it be", "It so be"),
        ("Fase 7", "Insisto que ela diga a verdade", "I insist that she tell the truth", "I insist she tells truth", "I insist that she telling truth", "I insist her to tell"),
        ("Fase 8", "Era importante que ele soubesse", "It was important that he know", "It was important that he knew", "It was important he is knowing", "It was important him know"),
    ]),
    ("Módulo 90: Discourse Markers", 3, [
        ("Fase 1", "Por outro lado", "On the other hand", "In the other hand", "By other hand", "From other hand"),
        ("Fase 2", "Apesar disso", "Despite that", "Despite of that", "In spite that", "Although that"),
        ("Fase 3", "Em outras palavras", "In other words", "In another words", "By other words", "With other words"),
        ("Fase 4", "Para concluir", "To conclude", "For conclude", "To concluding", "In concluding"),
        ("Fase 5", "Sem mencionar", "Not to mention", "Not mention", "Not for mention", "Without mention"),
        ("Fase 6", "Por exemplo", "For instance", "For example instance", "In instance", "By instance"),
        ("Fase 7", "Além disso", "Moreover", "More over", "Moroever", "More of"),
        ("Fase 8", "De qualquer forma", "In any case", "In any cases", "Any case in", "Anyway in case"),
    ]),
    ("Módulo 91: Idioms Avançados", 3, [
        ("Fase 1", "Ficar nas nuvens", "On cloud nine", "In cloud nine", "On the nine cloud", "On nine clouds"),
        ("Fase 2", "Atirar no escuro", "A shot in the dark", "Shot at dark", "Dark shot in", "Shoot in dark"),
        ("Fase 3", "Tirar da manga", "Pull out of a hat", "Pull from sleeve", "From sleeve pull", "Pull off hat"),
        ("Fase 4", "Cuspir no prato que comeu", "Bite the hand that feeds you", "Bite hand feeds", "Bite hand that feed", "Bite who feeds"),
        ("Fase 5", "Estar no mesmo barco", "Be in the same boat", "Be on same boat", "Be in the same boat all", "Same boat be in"),
        ("Fase 6", "Sem dó nem piedade", "No holds barred", "No hold barr", "Not hold barred", "Without hold bar"),
        ("Fase 7", "Pé na bunda (terminar)", "Give the boot", "Give a boot", "Give to boot", "Give boot to"),
        ("Fase 8", "Andar na linha", "Toe the line", "Walk on line", "Walk the line", "Toe on line"),
    ]),
    ("Módulo 92: Cleft Sentences", 3, [
        ("Fase 1", "Foi João quem me ajudou", "It was John who helped me", "Was John that helped me", "John was helped me", "It was John helped me"),
        ("Fase 2", "É de oração que precisamos", "What we need is prayer", "We need is prayer what", "Prayer is what need", "It's what we need prayer"),
        ("Fase 3", "Foi ontem que ela veio", "It was yesterday that she came", "Yesterday it was she came", "She came it was yesterday", "Was yesterday she came"),
        ("Fase 4", "O que eu mais gosto é cantar", "What I like most is singing", "What I most like is sing", "I like most is sing what", "Sing is what I like"),
        ("Fase 5", "Foi por isso que parei", "That's why I stopped", "Was that why I stopped", "Why I stopped that's was", "It's that I stopped why"),
        ("Fase 6", "Foi assim que aprendi", "That's how I learned", "Was how I learned", "How I learned that's was", "That is how learned I"),
        ("Fase 7", "É só Deus quem sabe", "It's only God who knows", "Only God knows it's", "It's God only knows who", "God's only knows who"),
        ("Fase 8", "Foi por amor que ele veio", "It was for love that he came", "Was for love he came", "He came was for love", "For love he came it was"),
    ]),
    ("Módulo 93: Vocabulário Avançado 1", 3, [
        ("Fase 1", "Ambíguo", "Ambiguous", "Ambigous", "Ambiguos", "Ambigius"),
        ("Fase 2", "Cético", "Skeptical", "Septical", "Skeptic", "Skepical"),
        ("Fase 3", "Inevitável", "Inevitable", "Inevitavle", "Unavoidable inev", "Inevitabel"),
        ("Fase 4", "Paradoxo", "Paradox", "Paradoks", "Paradoxe", "Paradocs"),
        ("Fase 5", "Plausível", "Plausible", "Plausivel", "Plausable", "Plauisble"),
        ("Fase 6", "Ressaltar", "Highlight", "High light", "Higlight", "Hihglight"),
        ("Fase 7", "Inferir", "Infer", "Inferr", "Infere", "Inferre"),
        ("Fase 8", "Imprescindível", "Indispensable", "Indispensavel", "Indispensible", "Indispensible"),
    ]),
    ("Módulo 94: Vocabulário Avançado 2", 3, [
        ("Fase 1", "Frugal (econômico)", "Frugal", "Frutal", "Frugall", "Frugel"),
        ("Fase 2", "Astuto", "Cunning", "Cuning", "Cuning sly", "Cunningly"),
        ("Fase 3", "Resiliente", "Resilient", "Resilent", "Resiliant", "Resillient"),
        ("Fase 4", "Empreendedor", "Entrepreneur", "Entrepeneur", "Entreprener", "Entreprenuer"),
        ("Fase 5", "Eloquente", "Eloquent", "Eloqent", "Elloquent", "Elocent"),
        ("Fase 6", "Sutil", "Subtle", "Subtile", "Sutle", "Subtel"),
        ("Fase 7", "Vislumbrar", "Glimpse", "Glimps", "Glimpse at", "Glipmse"),
        ("Fase 8", "Inerente", "Inherent", "Inherant", "Innerent", "Inerent"),
    ]),
]
...
def reset_para_inicio():
    ...
    Se a lição veio da trilha gamificada, volta pra trilha em vez da home."""
    # Se veio da trilha de módulo, volta pra lá (mantém imersão)
    if st.session_state.get("veio_de_trilha") and st.session_state.get("trilha_modulo_id"):
        st.session_state.tela = "trilha_modulo"
        st.session_state.veio_de_trilha = False  # consome o flag
    else:
...
# =========================================================
    ...
    if backend == "turso":
        ...
        # Se Turso estava configurado mas falhou, mostra o erro
        erro_turso_html = ""
        if _TURSO_ULTIMO_ERRO:
            erro_turso_html = (f"<br><br><b style='color:var(--danger);'>Erro ao conectar com Turso:</b> "
                               f"<code style='background:rgba(0,0,0,0.3);padding:2px 6px;border-radius:4px;'>"
                               f"{_TURSO_ULTIMO_ERRO}</code>")
        elif (_get_secret_or_env("TURSO_DATABASE_URL") or _get_secret_or_env("TURSO_AUTH_TOKEN")):
            erro_turso_html = ("<br><br><b style='color:var(--accent);'>Aviso:</b> "
                               "Você configurou só URL ou só TOKEN do Turso. Precisa dos dois.")
        ...
        st.markdown(
            ...
            "crie banco → adicione <code>TURSO_DATABASE_URL</code> e <code>TURSO_AUTH_TOKEN</code> nos Secrets."
            + erro_turso_html + "</span>"
            "</div></div>",
            unsafe_allow_html=True
        )
...
# =========================================================
    ...
    with c_main:
        ...
        for mid, mod_tit, nivel in modulos:
            ...
            ic_mod = icone_modulo(mod_tit)

            # Card do módulo com botão pra abrir a trilha gamificada
            barra_cls = "completo" if pct >= 100 else ("forte" if pct >= 50 else ("medio" if pct > 0 else ""))
            st.markdown(
                f"<div class='premium-card' style='display:flex;align-items:center;gap:14px;padding:16px;'>"
                f"<div style='font-size:2.2rem;'>{ic_mod}</div>"
                f"<div style='flex:1;min-width:0;'>"
                f"<div style='display:flex;align-items:center;gap:8px;flex-wrap:wrap;'>"
                f"<b style='font-family:Sora,sans-serif;font-size:1.05rem;'>{mod_tit}</b>"
                f"<span class='tag-nivel {niv_label[1]}'>{niv_label[0]}</span>"
                f"</div>"
                f"<div style='margin-top:6px;display:flex;align-items:center;gap:10px;'>"
                f"<div class='progress-wrap' style='flex:1;'><div class='progress-bar {barra_cls}' style='width:{pct}%;'></div></div>"
                f"<small style='color:var(--text-muted);white-space:nowrap;'>{feitas}/{total} ({pct}%)</small>"
                f"</div></div></div>",
                unsafe_allow_html=True
            )
            if total == 0:
                ...
                if st.button(f"🎮 Abrir trilha", key=f"abrir_trilha_{mid}", use_container_width=True):
                    st.session_state.trilha_modulo_id = mid
                    st.session_state.tela = "trilha_modulo"
                    st.rerun()
...
# =========================================================
# TELA: TRILHA GAMIFICADA DE UM MÓDULO (estilo Duolingo)
# =========================================================
elif st.session_state.tela == "trilha_modulo":
    uid = st.session_state.uid
    mid = st.session_state.get("trilha_modulo_id")

    if not mid:
        st.warning("Nenhum módulo selecionado.")
        if st.button("⬅️ Voltar pra home"):
            st.session_state.tela = "inicio"; st.rerun()
    else:
        # Header e dados do módulo
        mod_info = consultar_um("SELECT titulo, nivel FROM modulos WHERE id = ?", (mid,))
        if not mod_info:
            st.error("Módulo não encontrado.")
            if st.button("⬅️ Voltar"):
                st.session_state.tela = "inicio"; st.rerun()
        else:
            mod_titulo, mod_nivel = mod_info
            ic_mod = icone_modulo(mod_titulo)
            niv_label = {1: ("Básico", "nivel-1"), 2: ("Intermediário", "nivel-2"), 3: ("Avançado", "nivel-3")}.get(mod_nivel, ("—", "nivel-1"))
            ...
            licoes_mod = consultar("SELECT id, titulo_botao FROM licoes WHERE modulo_id = ? ORDER BY id", (mid,))
            total = len(licoes_mod)
            feitas_ids = {r[0] for r in consultar(
                "SELECT licao_id FROM progresso WHERE aluno_id = ? AND licao_id IN ("
                + ",".join("?" * len(licoes_mod)) + ")",
                tuple([uid] + [l[0] for l in licoes_mod])
            )} if licoes_mod else set()
            feitas = len(feitas_ids)
            pct = int(100 * feitas / total) if total else 0

            # Header bonito do módulo
            st.markdown(
                f"<div class='premium-card' style='display:flex;align-items:center;gap:16px;padding:18px;margin-bottom:8px;'>"
                f"<div style='font-size:3rem;'>{ic_mod}</div>"
                f"<div style='flex:1;'>"
                f"<h1 style='margin:0;font-family:Sora,sans-serif;font-size:1.5rem;'>{mod_titulo}</h1>"
                f"<div style='margin-top:6px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;'>"
                f"<span class='tag-nivel {niv_label[1]}'>{niv_label[0]}</span>"
                f"<small style='color:var(--text-muted);'>{feitas}/{total} lições completadas ({pct}%)</small>"
                f"</div>"
                f"<div class='progress-wrap' style='margin-top:8px;'>"
                f"<div class='progress-bar {'completo' if pct >= 100 else ('forte' if pct >= 50 else ('medio' if pct > 0 else ''))}' style='width:{pct}%;'></div>"
                f"</div></div></div>",
                unsafe_allow_html=True
            )
...
""")
                ...
                if feitas == total and total > 0:
                    st.markdown("<br>", unsafe_allow_html=True)
                    st.success(f"🎉 **Parabéns!** Você completou todo o módulo *{mod_titulo}*. Procure outro pra continuar aprendendo!")
